...
class TrajectoryTests extends AnyFlatSpec with should.Matchers {
  ...
  "Duplicate points" should "be handled" in {
    val c1 = Float2(0, -5)
    val c2 = Float2(50, 0)
    val c3 = Float2(50, 30)
    val c4 = Float2(0, 30)

    val e1 = linSpace(c1, c2, 10).dropRight(1)
    val e2 = linSpace(c2, c3, 10).dropRight(1)
    val e3 = linSpace(c3, c4, 10).dropRight(1)
    val e4 = linSpace(c4, c1, 10)

    // Add duplicates
    e1(3) = e1(4); e1(2) = e1(4); // middle
    e3(3) = e3(0); e3(2) = e3(0); e3(1) = e3(0); // corner
